...
Derek (x7c1b):
I worked on both front-end and back-end as well as implmeneting log in across front-end and back-end. On the back-end side, I helped set up user end points. Here I primarily work on the end points that will send an API call to back-end the moment user logs in, including verification on the back-end. On the front end side, I set up redux so that we can store import data in store and have it be accessible across all our components(ex: cartQuantity). I also write search functions so that users can look up for the furniture he/she wants. Finally, I set up the payment system using PayPal to allow for transactions in our web app.
